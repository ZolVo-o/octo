...
Launcher `octo` написан на Bash и передаёт аргументы C++ backend. Rust TUI
сохранён в исходниках, но текущий launcher его не запускает.

### Go Terminal

В репозитории ведётся отдельная разработка собственного терминала на Go. Это не
обёртка над Alacritty или Kitty: терминал использует собственную модель экрана,
ANSI-эмулятор, PTY-сессию, ввод, рендеринг и обработку resize.

Текущие возможности экспериментального терминала:

- интерактивная shell-сессия через PTY;
- собственный ANSI-парсер с курсором, цветами, очисткой областей и управляющими
  последовательностями;
- UTF-8-ввод и вывод;
- scrollback-история с колесом мыши, `PageUp`, `PageDown`, `Ctrl+Home` и
  `Ctrl+End`;
- голубой индикатор положения в scrollback;
- выделение мышью и копирование через `Ctrl+C`;
- вставка из системного буфера через `Ctrl+V`;
- история введённых команд через `↑` и `↓`;
- отдельная нижняя панель ввода с prompt `Octo terminal ]$`;
- фоновое изображение, иконка окна и выбор шрифта из `assets`;
- нейтральная голубая палитра без анимаций и декоративных эффектов.

> Go Terminal находится в разработке. Его интерфейс, ANSI-совместимость,
> Unicode-ширины и некоторые детали поведения ещё будут дорабатываться.
> Текущий релиз OCTO не зависит от этого экспериментального модуля.

Проверка Go Terminal выполняется из его каталога:

```bash
cd terminal-go
go test ./...
go vet ./...
go build -o target/octo-terminal .
```

При будущей публикации текущая рабочая папка `terminal-go` не включается в
push основного репозитория, пока терминал не будет доведён до готового релиза.

### Пакетные функции

- установка одного или нескольких пакетов из официальных репозиториев;
- установка пакетов из AUR;
- удаление пакетов и зависимостей;
- поиск по официальным репозиториям и AUR;
- обновление системы;
- информация о пакете и его размере;
- список пакетов, история операций и SQLite-база;
- автоматические текстовые бэкапы перед изменениями;
- диагностика зависимостей и окружения;
- проверка PKGBUILD;
- мониторинг и сетевой бенчмарк;
- собственный OCTO-вывод без успешного вывода `pacman`.

Набор доступных операций зависит от установленных в системе утилит. Полный список
публичных команд показывает `octo help`.
...
В сохранённом Rust TUI экран, ввод и маршрутизация команд разделены с backend.
В текущем CLI launcher аргументы передаются напрямую в C++ backend:

```text
CLI launcher → ./octo → target/octo-backend <command> <args>
```
...
Текущий launcher развивает CLI и C++ backend пакетного менеджера; исходники
Rust TUI сохранены для последующего возвращения в основной поток запуска.
...
Исходники TUI сохраняются в репозитории, но launcher и `PKGBUILD` сейчас не
запускают и не устанавливают TUI как основной интерфейс. Текущий рабочий
интерфейс — CLI backend.
...
Для официальных пакетов OCTO скрывает успешный `stdout` и `stderr` `pacman` и
показывает собственные этапы:

```text
🐙 Щупальца тянутся к официальному репозиторию...
🐙 Проверяет зависимости... ✅
🐙 Проверяет конфликты... ✅
🐙 Скачивает пакет... | 🐙🐙🐙🐙🐙🐙🐙🐙🐙🐙 | 100% |
🐙 Проверяет ключи... ✅
🐙 Устанавливает пакет... ✅ (12.4s)
🐙 Завершает установку... ✅
✅ Пакеты пойманы! 🎉
```

В случае ошибки вывод `pacman` не теряется: OCTO показывает код завершения и
захваченный текст ошибки в диагностическом блоке.

Обычный CLI поддерживает несколько официальных пакетов:

```bash
octo install nodejs npm
octo установить nodejs npm
```

Интерфейс такой операции:

```text
📦 Пакеты: nodejs, npm [офиц.]
🐙 nodejs, npm установлены!
```

Размер пакета выводится в осьминожьем формате:
...
🐙 Попробуй: octo щупальца python
```

## CLI SHELL

В исходной версии TUI ввод отображается в формате:
...
Обычный launcher поддерживает несколько официальных пакетов, но текущий Rust
CLI SHELL нормализует установку только одного пакета. Для нескольких пакетов
используйте `octo install nodejs npm` вне TUI shell.
...
Не удаляйте этот каталог вручную, если хотите сохранить базу, историю и бэкапы.
...
- собственные этапы установки, таймер и скрытие успешного вывода `pacman`;
- установка нескольких официальных пакетов через обычный CLI;
- единый формат ошибок с подсказкой поиска в AUR.
